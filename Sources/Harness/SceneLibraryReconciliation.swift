...
extension SceneLibraryStore {
    ...
    static func boundedDigests(root: URL, drafts: [SourceEntry], indices: [Int],
        ...
        for index in selected {
            ...
            let values = try resolvedURL.resourceValues(forKeys: [.isRegularFileKey, .fileSizeKey, .contentModificationDateKey])
    ...
    private static func probableObservationIdentity(_ observation: ReconciliationObservation?) -> String? {
        guard let observation else { return nil }
        if let bytes = observation.byteLength { return "bytes:\(bytes)" }
        if let revision = normalizedIdentity(observation.packageRevision) { return "package:\(revision)" }
        return nil
    }

    private static func probableSignature(entry: Entry) -> String? {
        guard let observationIdentity = probableObservationIdentity(entry.observation) else { return nil }
        ...
        return "\(observationIdentity)|\(type)|\(dimensions)|\(duration)|\(title)"
    }

    private static func probableSignature(draft: SourceEntry) -> String? {
        guard let observationIdentity = probableObservationIdentity(draft.observation) else { return nil }
    ...
    private static func probableEvidence(entry: Entry, draft: SourceEntry) -> String {
        var evidence: [String] = []
        if let bytes = entry.observation?.byteLength, bytes == draft.observation?.byteLength { evidence.append("same byte length") }
        if let revision = normalizedIdentity(entry.observation?.packageRevision),
           revision == normalizedIdentity(draft.observation?.packageRevision) { evidence.append("same package revision") }
