...
/// One bounded dispatch point shared by every external automation surface.
/// App/URL/CLI/Shortcut adapters provide current controller operations through
/// `Dependencies`; parsing and transport never grow their own actuation logic.
@MainActor
final class AutomationCommandExecutor {
    ...
    func execute(_ rawCommand: AutomationCommand, requestID: UUID = UUID()) async -> AutomationResponse {
        do {
            ...
            return AutomationResponse(id: requestID, success: false, message: error.localizedDescription,
                                      state: dependencies.currentState())
        }
    }
}
