...
# 8. Wallpaper display/coverage policy and the shared raw attention mapping.
WALLPAPER_POLICY_SRCS=(
  Sources/Wallpaper/CoverageRestPolicy.swift
  Sources/Wallpaper/CoverageMonitor.swift
  Sources/Wallpaper/DesktopAttentionSignal.swift
...
# 9. Scene-level desktop-attention capability, binding evaluation and smoothing.
DESKTOP_ATTENTION_SRCS=(
  Sources/Runtime/Scene.swift
  Tests/DesktopAttentionTests.swift
)
if needs_build "build/tests/desktop-attention" "${DESKTOP_ATTENTION_SRCS[@]}"; then
  xcrun swiftc "$OPT_FLAG" "${DESKTOP_ATTENTION_SRCS[@]}" -o build/tests/desktop-attention &
  pids+=($!)
fi
