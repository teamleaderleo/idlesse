...
mkdir -p "$cache"
xcrun swiftc -parse-as-library -target arm64-apple-macos26.0 -module-cache-path "$cache" -import-objc-header Experiments/NativeWallpaper/Bridge.h Experiments/NativeWallpaper/Probe.swift Experiments/NativeWallpaper/Surface.swift Experiments/NativeWallpaper/Catalog.swift Experiments/NativeWallpaper/Vendor/CodableShims.swift -framework QuartzCore -framework IOSurface -framework AppKit -o "$app/Contents/MacOS/NativeProbe"
...
"$app/Contents/MacOS/NativeProbe" --surface-check > "$output/surface-check.txt"
