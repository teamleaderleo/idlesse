...
@main struct StudioMotionTests {
    static func main() throws {
        ...
        try typedControlRoundTrip()
        try incompatibleTypedControlIsRejected()
    ...
    private static func typedControlRoundTrip() throws {
        let layer = SceneNode(content: .shape(.init(fill: "#224466")), visible: true)
        let visible = SceneControlTarget(nodeID: layer.id, property: .visible)
        let fill = SceneControlTarget(nodeID: layer.id, property: .fill)
        var scene = SceneDescriptor(title: "Typed", nodes: [layer], parameters: [
            "show": .init(name: "Show", type: .boolean, boolean: false),
            "accent": .init(name: "Accent", type: .color, text: "#AA7733")
        ])
        scene = try StudioMotionAuthoring.bind(visible, toParameter: "show", in: scene)
        scene = try StudioMotionAuthoring.bind(fill, toParameter: "accent", in: scene)
        precondition(StudioMotionAuthoring.typedOwnership(of: visible, in: scene) == .controlled("show"))
        precondition(StudioMotionAuthoring.typedOwnership(of: fill, in: scene) == .controlled("accent"))
        let bakedVisible = try StudioMotionAuthoring.makeStatic(visible, in: scene)
        let bakedNode = bakedVisible.allNodes[0]
        precondition(bakedNode.visible == false)
        precondition(StudioMotionAuthoring.typedOwnership(of: visible, in: bakedVisible) == .staticValue)
        precondition(bakedVisible.parameters["show"] != nil, "owner changes keep controls")
        precondition(StudioMotionAuthoring.typedOwnership(of: fill, in: bakedVisible) == .controlled("accent"))
    }

    private static func incompatibleTypedControlIsRejected() throws {
        let layer = node()
        let target = SceneControlTarget(nodeID: layer.id, property: .visible)
        let scene = SceneDescriptor(title: "TypedMismatch", nodes: [layer], parameters: [
            "gain": .init(name: "Gain", value: 1, min: 0, max: 2)
        ])
        do {
            _ = try StudioMotionAuthoring.bind(target, toParameter: "gain", in: scene)
            preconditionFailure("boolean ownership should reject a numeric control")
        } catch { }
    }
}
